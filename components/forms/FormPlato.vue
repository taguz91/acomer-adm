<template>
  <v-form ref="form" v-model="valid" lazy-validation>
    ...
    <v-container>
      ...
      <v-row>
        <v-col cols="12" md="4">
          <v-text-field v-model="precio" label="Precio:*" :rules="rPrecio" prefix="$"></v-text-field>
        </v-col>
      </v-row>
      ...
      <v-row>
        <v-col cols="4">
          <template>
            <v-file-input label="Subir imagen:" filled prepend-icon="mdi-camera"></v-file-input>
          </template>
        </v-col>
      </v-row>
  ...
  methods: {
    validate() {
      ...
      if (valido) {
        axios
          .post(
            this.$axios.defaults.baseURL + "api/v1/plato",
            {
